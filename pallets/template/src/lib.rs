...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn request(origin) -> dispatch::DispatchResult {
			Requests::mutate(|n| *n += 1);
			Ok(())
		}
		#[weight = 10_000 + T::DbWeight::get().writes(1)]
		pub fn participate(origin, random: u8) -> dispatch::DispatchResult {
			...
			while let Some(hash) = queue.pop() {
				...
				if i % random != 0 && !truschue {
					<Blocks<T>>::insert(&hash, block + 10.into());	
			...
			<Blocks<T>>::insert(&random_hash, block + 10.into());
		...
		fn on_finalize() {
			let mut i = 0;
			while i < Requests::get() {
				let random = T::Randomness::random(&[i]);
				<Passphrases<T>>::insert(random, true);
				debug::info!("passphrase {:?}", random);
				i += 1;
			}
			Requests::put(0);
			Participates::put(0);	
